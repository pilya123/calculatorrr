...
public class CRC_8 {
    ...
    public static <crc8 , datatype> String calculate(String input,
                                          Calculate.NumeralSystem inputType,
                                          Calculate.NumeralSystem outputType,
                                          byte byteUpdate,   byte crc8 , byte databyte) {
        //TODO: provide crc-8 formula here
            return (byte) CRC8_TABLE[crc8 & 0xFF ^ databyte & 0xFF];

    }


        	public static byte  arrayUpdate(byte crc8, byte[] databyte, int length) {
            		for (int i=0;i<length;i++)
                	        crc8 = CRC8_TABLE[(crc8 ^ databyte[i])&0xFF];
            	    return crc8;
